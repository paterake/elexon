package com.paterake.elexon.model;

import javax.xml.bind.annotation.XmlElement;
import javax.xml.bind.annotation.XmlRootElement;

@XmlRootElement(name = "item")
public class DataBodyB1610 extends DataBody {

    @XmlElement(name = "documentType")
    public String documentType;

    @XmlElement(name = "businessType")
    public String businessType;

    @XmlElement(name = "processType")
    public String processType;

    @XmlElement(name = "timeSeriesID")
    public String timeSeriesID;

    @XmlElement(name = "quantity")
    public String quantity;

    @XmlElement(name = "curveType")
    public String curveType;

    @XmlElement(name = "resolution")
    public String resolution;

    @XmlElement(name = "settlementDate")
    public String settlementDate;

    @XmlElement(name = "settlementPeriod")
    public String settlementPeriod;

    @XmlElement(name = "powerSystemResourceType")
    public String powerSystemResourceType;

    @XmlElement(name = "registeredResourceEICCode")
    public String registeredResourceEICCode;

    @XmlElement(name = "marketGenerationUnitEICCode")
    public String marketGenerationUnitEICCode;

    @XmlElement(name = "marketGenerationBMUId")
    public String marketGenerationBMUId;

    @XmlElement(name = "marketGenerationNGCBMUId")
    public String marketGenerationNGCBMUId;

    @XmlElement(name = "bMUnitID")
    public String bMUnitID;

    @XmlElement(name = "nGCBMUnitID")
    public String nGCBMUnitID;

    @XmlElement(name = "activeFlag")
    public String activeFlag;

    @XmlElement(name = "documentID")
    public String documentID;

    @XmlElement(name = "documentRevNum")
    public String documentRevNum;

    @Override
    public String toString() {
        return documentType
                + ":" + businessType
                + ":" + processType
                + ":" + timeSeriesID
                + ":" + quantity
                + ":" + curveType
                + ":" + resolution
                + ":" + settlementDate
                + ":" + settlementPeriod
                + ":" + powerSystemResourceType
                + ":" + registeredResourceEICCode
                + ":" + marketGenerationUnitEICCode
                + ":" + marketGenerationBMUId
                + ":" + marketGenerationNGCBMUId
                + ":" + bMUnitID
                + ":" + nGCBMUnitID
                + ":" + activeFlag
                + ":" + documentID
                + ":" + documentRevNum
                ;
    }
}
